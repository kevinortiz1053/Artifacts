...
def sarsa(q, episodes, alpha):
    ...
    plt.title('Sarsa (baseline), alpha = ' + str(alpha))
...
optimal_q = sarsa({}, 10000, 0.5)
with open('sarsa_baseline.pkl', 'wb') as fp:
    pickle.dump(optimal_q, fp)
...
def averaged_run():
    ...
    success_array = []
    for k in range(10):
        num_of_successes_array = sarsa({}, 10000, alpha)
    ...
    plt.title("(no seed) Sarsa (baseline), " + 'alpha = ' + str(alpha))
    plt.xlabel('Number of Episodes')
