...
def get_rewards(sample_mean, step_mean):
    reward_sample1 = np.random.normal(loc=sample_mean, scale=1)
    reward_step1 = np.random.normal(loc=step_mean, scale=1)
...
for k in range(2000):  # Number of runs
    ...
    while t < 10000:  # Number of steps per run
        ...
        reward_sample, reward_step = get_rewards(init_q_sample[action_index_sample], init_q_step[action_index_step])
